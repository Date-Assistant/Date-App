...
for ip in active_hosts:
    if 'RabbitMQ' in active_hosts[ip]:
        ...
        # credentials to log into rabbit
        credentials = pika.PlainCredentials('thebigrabbit','it490')
        # parameters take ip addr, port, vhost (virtual host that can access
        # rabbitmq; can assign multiple users to a single vhost), and credentials
        parameters = pika.ConnectionParameters(ip,5672,'cherry_broker',credentials)
        # creates connection (pass in connection parameters)
        connection = pika.BlockingConnection(parameters)
        # channel to send queue
        channel = connection.channel()
        # queue name that gets passed in for receiving
        ...
        connection = pika.BlockingConnection(parameters)
        #channel creation to receive queues
        channel = connection.channel()
        # channel.basic_get(queue name that was sent above)
        # body will be the body that was passed in
